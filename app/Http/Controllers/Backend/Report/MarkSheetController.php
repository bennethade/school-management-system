...
class MarkSheetController extends Controller
{
    ...
    public function markSheetGet(Request $request)
    {
        ...
        $count_fail = StudentMarks::where('year_id',$year_id)->where('class_id',$class_id)->where('exam_type_id',$exam_type_id)->where('id_no',$id_no)->where('exam','<','45')->get()->count();
        //Where the database field name matches with the requested field name from the view file
        // dd($count_fail);
        ...
        if ($singleStudent == true) {
            ...
            // $allSubjects = AssignSubject::with(['assign_subject'])->where('class_id',$class_id)->get();
            // dd($allMarks->toArray());
